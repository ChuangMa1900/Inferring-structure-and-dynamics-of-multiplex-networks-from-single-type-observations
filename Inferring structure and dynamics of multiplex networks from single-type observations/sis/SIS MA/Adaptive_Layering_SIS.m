function [w_r,E_r]=Adaptive_Layering_SIS(X,N,L,c1,c2,S,Gamma_0)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%重构后的自适应分层算法，多组结果供选择
...
for i=1:n1 
    ...
    E_L=-Likelihood_Fun_Value_SIS(w_r1,Z1,S); %负的似然
    E_P=abs(prod(exp(Z1).^((reshape(sum(w_r1,1),[N,L])./(N-1))),2)-Gamma_0); %参数一致性误差越小越好
    E_P=sum(E_P);
