function [w_r,E_r]=Adaptive_Layering_Ising(X,N,L,c1,c2,S,Gamma_0)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%重构后的自适应分层算法，多组结果供选择
...
E_r=zeros(n1,4);  %存储4个误差指标

kk=0;
for i=1:n1 
    ZZ=Z0(i,:);   
    [A1,Z1,E0]=Fun_Layering(X,L,ZZ);
    ...
    E_L=-Likelihood_Fun_Value_Ising(w_r1,Z1,S); %负的似然
    E_P=abs(((reshape(sum(w_r1,2),[N,L])*Z1')/(N-1))-Gamma_0); %越小越好
    E_P=sum(E_P);

    E_r(kk,:)=[E0,E_P,E_L,E_S];
end
w_r=w_r(1:kk,:);
E_r=E_r(1:kk,:);
